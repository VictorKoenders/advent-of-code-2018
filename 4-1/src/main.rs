...
fn main() {
    ...
    let mut highest_guard = (0, 0, 0);

    for (id, pattern) in guard_sleep_pattern {
        let sleep_amount = pattern.iter().fold(0, |acc, val| acc + val);
        let (most_sleep_minute, most_sleep_minute_amount) = pattern.iter().enumerate().max_by_key(|x| x.1).unwrap();
        println!("Guard {} slept {} minutes", id, sleep_amount);
        println!("They slept the most at minute {}, {} times", most_sleep_minute, most_sleep_minute_amount);
        if sleep_amount > highest_guard.0 {
            highest_guard = (
                sleep_amount,
                id,
                most_sleep_minute
    ...
    println!("I choose guard {}, which slept for {} minutes, with the most likely minute to be {}",
             highest_guard.1, highest_guard.0, highest_guard.2);
    println!("{} x {} = {}", highest_guard.1, highest_guard.2, highest_guard.1 as usize * highest_guard.2);
}
